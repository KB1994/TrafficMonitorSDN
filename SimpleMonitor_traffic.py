...
class SimpleMonitor13(simple_switch_13.SimpleSwitch13):
	...
	def port_desc_stats_reply_handler(self, ev):
		...
		for p in ev.msg.body:
			...
			self.hwd_name[p.hw_addr] = p.name
	...
	def show_stat(self, _type):
		'''
			...
			_type: 'port' / 'flow'
		'''
		with open('Network_monitoring_trace.txt' , 'a') as f:
			f.write('record_time : '+ datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S"))
			f.write('\n')
    

			if self.TOSHOW is False:
				return
			hard_name = self.hwd_name
			#print(hard_name)
			bodys = self.stats[_type]
			if _type == 'flow':
				print('    datapath     ''   in-port       eth-src              eth-dst       '' packets'' name_src'' name_dest')
				f.write('    datapath     ''   in-port       eth-src              eth-dst       '' packets'' name_src'' name_dest')
				f.write('\n')
				print('---------------- ''  -------- ''-----------------   ----------------- '' ---------'' ---------'' ---------')
				f.write('---------------- ''  -------- ''-----------------   ----------------- '' ---------'' ---------'' ---------')
				f.write('\n')
				for dpid in bodys.keys():
					for stat in sorted([flow for flow in bodys[dpid] if flow.priority == 1],
						...
						print('%016x %8x %19s %19s %8d %8s %8s' % (
						dpid,
						stat.match.get('in_port'), stat.match.get('eth_src'), stat.match.get('eth_dst'),
						stat.packet_count, hard_name[stat.match.get('eth_src')] if stat.match.get('eth_src') in list(hard_name.keys()) else 'NA', hard_name[stat.match.get('eth_dst')] if stat.match.get('eth_dst') in list(hard_name.keys()) else 'NA'))
						print
						f.write('%016x %8x %19s %19s %8d %8s %8s' % (
						dpid,
						stat.match.get('in_port'), stat.match.get('eth_src'), stat.match.get('eth_dst'),
						stat.packet_count, hard_name[stat.match.get('eth_src')] if stat.match.get('eth_src') in list(hard_name.keys()) else 'NA', hard_name[stat.match.get('eth_dst')] if stat.match.get('eth_dst') in list(hard_name.keys()) else 'NA'))
						f.write('\n')
